...
bool Cmp_add_op_s::isDeclared = false;

Cmp_add_op_s::Cmp_add_op_s(int dataWidth) : Component() {
    this->setComponentName("add_op_s");
    this->setDataWidth(dataWidth);
    this->createPorts();
    this->createGenericMaps();
}

void Cmp_add_op_s::setIsDeclared(bool isDeclared) {
    this->isDeclared = isDeclared;
}

bool Cmp_add_op_s::getIsDeclared() {
    return isDeclared;
}

// Returns the vhdl decl of the component
std::string Cmp_add_op_s::getVHDLDeclaration(std::string nameExtension){
    std::string decl = "";
    
    decl += "component add_op_s\n";  
    decl += "generic (\n";
...
void Cmp_add_op_s::createGenericMaps() {
