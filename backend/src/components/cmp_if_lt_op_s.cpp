...
bool Cmp_if_lt_op_s::isDeclared = false;

Cmp_if_lt_op_s::Cmp_if_lt_op_s(int dataWidth) : Component() {
    this->setComponentName("if_lt_op_s");
    this->setDataWidth(dataWidth);
    this->createPorts();
    this->createGenericMaps();
}

void Cmp_if_lt_op_s::setIsDeclared(bool isDeclared) {
    this->isDeclared = isDeclared;
}

bool Cmp_if_lt_op_s::getIsDeclared() {
    return isDeclared;
}

// Returns the vhdl decl of the component
std::string Cmp_if_lt_op_s::getVHDLDeclaration(std::string nameExtension){
    std::string decl = "";
    
    decl += "component if_lt_op_s\n";  
    decl += "generic (\n";
